  ...
  for (let i = 0; i < bullets.length; i++) {
    ...
    if (bullets[i].visible == true) {
      ...
      for (let x = 0; x < enemy.length; x++) {
        ...
        if (
          enemy[x].y + enemy[x].h >= bullets[i].y &&
          ((bullets[i].x >= enemy[x].x &&
            bullets[i].x <= enemy[x].x + enemy[x].w) ||
            (bullets[i].x + bullets[i].w <= enemy[x].x + enemy[x].w &&
              bullets[i].x + bullets[i].w >= enemy[x].x))
        ) {
